...
# For renoir build
if [ ${DEVICE} == "renoir" ]; then
XIAOMI_RENOIR_QGKI_FRAG=${CONFIGS_DIR}/renoir_QGKI.config
fi
...
export ARCH CROSS_COMPILE REAL_CC HOSTCC HOSTLD HOSTAR KERN_SRC KERN_OUT \
	CONFIGS_DIR BASE_DEFCONFIG XIAOMI_QGKI_FRAG XIAOMI_RENOIR_QGKI_FRAG QCOM_GKI_FRAG QCOM_QGKI_FRAG QCOM_DEBUG_FRAG
else
export ARCH CROSS_COMPILE REAL_CC HOSTCC HOSTLD HOSTAR KERN_SRC KERN_OUT \
	CONFIGS_DIR BASE_DEFCONFIG XIAOMI_QGKI_FRAG QCOM_GKI_FRAG QCOM_QGKI_FRAG QCOM_DEBUG_FRAG
fi